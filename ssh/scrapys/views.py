...
def fjob(request):
    if request.method == "GET":
        ...
        is_exist = AnsysFile.objects.filter(title=search_name)
        if not is_exist:
            try:
                scraps = scrapy(search_name)
                file_name,data_name = scraps.get_data()
                handles = handle(search_name, file_name)
                data_tran,data_final = handles.get_result()
                file = AnsysFile(title=search_name,file_name=file_name,data_name=data_name,data_tran=data_tran,data_final=data_final)
                file.save()
            except:
                error = "error"
            return HttpResponseRedirect(reverse('scrapys_search'))
        file_ = os.path.join(BASE_DIR,'scrapys/',is_exist.data_final)
