 ...
 */
vector<string> Files::get_files(vector<string> &files_list,
    ...
    return (this->*s_Model[read_model])(dir_path, files_list);
}


/* 文件格式是否匹配 */
template <typename file_t>
bool _patt_in_name(file_t &file_info) {
...
vector<string> Files::get_full_path(string path, vector<string> &files_list) {
    ...
    return vector<string>(files_list);
}
...
vector<string> Files::get_cur_dir(string path, vector<string> &files_list) {
    long file_handle = 0; 
    struct _finddata_t file_info;
    string p; 
    if ((file_handle = _findfirst(p.assign(path).append("\\*").c_str(), &file_info)) != -1) {
