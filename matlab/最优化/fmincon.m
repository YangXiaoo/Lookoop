...
%   $Revision: 1.1.6.25 $  $Date: 2012/05/08 20:38:51 $
...
% sizes.xCols=1
XOUT = X(:); % XOUT = 200 100
sizes.nVar = length(XOUT); %sizes.nVar=2
% Check for empty X
if sizes.nVar == 0
...
if isempty(Aeq)
    Aeq = reshape(Aeq,0,sizes.nVar); % Empty matrix: 0-by-2 
    % Aeq 为空矩阵0x2
end
if isempty(A)
...
% Used to transition default algorithm from trust-region-reflective to
% interior-point-convex:
AlgorithmUnsetByUser = ~isfield(options,'Algorithm') || isempty(optimget(options,'Algorithm')); 
...
if strcmpi(OUTPUT.algorithm,activeSet) || strcmpi(OUTPUT.algorithm,sqp)
    ...
    if algChoiceOptsConflict && strcmpi(OUTPUT.algorithm,activeSet)
        ...
        warning(message('optimlib:fmincon:NLPAlgLargeScaleConflict'))
        if AlgorithmUnsetByUser
            % Warn that will run a different algorithm (interior-point) in a future release.
            [linkTag,endLinkTag] = linkToAlgDefaultChangeCsh('fmincon_warnN1'); % links to context sensitive help
            warning(message('optimlib:fmincon:WillRunDiffAlg',linkTag,endLinkTag,'interior-point'));            
        else
            if flags.grad && (isBoundedNLP || isLinEqNLP)
                % Problem can be handled by trust-region-reflective, LargeScale=off, Algorithm = trr. 
                % Running active-set because honoring LargeScale. Warn that LargeScale will 
                % be ignored in favor of Algorithm. Thus will run trust-region-reflective in a 
                % future release
                [linkTag,endLinkTag] = linkToAlgDefaultChangeCsh('fmincon_warnN3'); % links to context sensitive help
                warning(message('optimlib:fmincon:WillRunTrr',linkTag,endLinkTag,'trust-region-reflective'));
            else
                % Warn that will error in a future release. (User chose trust-region-reflective for
                % a problem that this algorithm can't handle.)
                [linkTag,endLinkTag] = linkToAlgDefaultChangeCsh('fmincon_warnN2'); % links to context sensitive help
                warning(message('optimlib:fmincon:TrrWillError',linkTag,endLinkTag));
            end
        end
    end
...
elseif strcmpi(OUTPUT.algorithm,trustRegionReflective)
    ...
    elseif isLinEqNLP && flags.grad 
        ...
        if ~isBoundedNLP && ~isLinEqNLP
            % The user has specified a problem which doesn't satisfy the
            % restrictions on the constraints for the
            % trust-region-reflective (TRR) algorithm. We tell the user
            % that TRR cannot solve their problem. This case is checked for
            % first because we believe it to be the most common way users
            ...
            % restrictions but they haven't supplied gradients. We believe
            % this case to happen less often.
        ...
        if strcmpi(funfcn{1},'fungradhess')
            funfcn{1}='fungrad';
            warning(message('optimlib:fmincon:HessianIgnored'))
        elseif  strcmpi(funfcn{1},'fun_then_grad_then_hess')
            funfcn{1}='fun_then_grad';
            warning(message('optimlib:fmincon:HessianIgnored'))
        end
...
elseif strcmpi(OUTPUT.algorithm,trustRegionReflective) % trust-region-reflective
   ...
   if AlgorithmUnsetByUser
       ...
       if flags.hess
           % Warn that trust-region-reflective Hessian or Hessian-multiply will have to be updated for it to work 
           % with interior-point.
           [linkTag,endLinkTag] = linkToAlgDefaultChangeCsh('fmincon_warnN4'); % links to context sensitive help 
           warning(message('optimlib:fmincon:UpdateHess',linkTag,endLinkTag,'trust-region-reflective'));
       end
   end
   if isempty(Aeq)
      [X,FVAL,LAMBDA,EXITFLAG,OUTPUT,GRAD,HESSIAN] = ...
...
elseif strcmpi(OUTPUT.algorithm,interiorPoint)
    ...
    [options,optionFeedback] = getIpOptions(options,sizes.nVar,mEq,flags.constr,defaultopt,10,0.01); 
    ...
    [options,optionFeedback] = getSQPOptions(options,defaultopt,sizes.nVar);
