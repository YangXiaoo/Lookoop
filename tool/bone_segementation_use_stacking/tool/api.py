...
def saveError(e, out_dir, f, logger=None):
    """
    ...
    if logger:
        ...
        logger.error("failed to handle %s, skiped.\nsaved in %s" % (f,failed_dir))
        logger.error("path: %s" % os.path.join(failed_dir, f.split("\\")[-1]))
    else:
        print("Error: " + str(e)) 
...
def printEst(res, way, logger=None):
    """
    打印分割评估结果
