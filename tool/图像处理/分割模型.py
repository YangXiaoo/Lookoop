...
def batchProcess(file_path_1, file_path_2):
	...
	for i in range(len_files):
		accuracy_rate, error_rate, loss_rate = getAccuracy(files[i], files_2[i])
		pic_name = files_1[i].split("\\").split(".")[0] # 得到图片名称
...
if __name__ == '__main__':
	file_path = "xxxx" # 标准分割图像目录路径
	file_path_1 = "xxxx" # 方法一得到分割图像路径

	# 方法一比较
	res = batchProcess(file_path, file_path_1)
	for k, v in res.items():
		print("图片:%s的分割精度为%f, 过分割率为%f, 欠分割率为:%f" % (k, v[0], v[1], v[2]))
